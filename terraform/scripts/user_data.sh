...
sudo chown ubuntu:ubuntu /mnt/efs/data
sudo chmod 755 /mnt/efs/data

# Generate files as the ubuntu user (not root)
cd /mnt/efs/data
for i in {1..10}; do
  base64 /dev/urandom | head -c 200 > "file_$i.txt"
done

# Set proper ownership on all files
sudo chown -R ubuntu:ubuntu /mnt/efs/data
sudo chmod -R 644 /mnt/efs/data/*.txt

# Optionally, make the mount persistent across reboots
